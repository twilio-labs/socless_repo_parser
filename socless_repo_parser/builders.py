...
class IntegrationFamilyBuilder:
    def build_from_github(self, gh_repo: Repository) -> IntegrationFamily:
        ...
        return self._parse_lambdas_and_build_family(parsed_yml, raw_lambda_files)
    ...
    def build_from_local(self, local_dir_path: str) -> IntegrationFamily:
        ...
        raw_lambda_files = self._fetch_raw_lambda_files_from_local(parsed_yml)
        return self._parse_lambdas_and_build_family(parsed_yml, raw_lambda_files)

    def _parse_lambdas_and_build_family(
        self, parsed_yml: SoclessParsedYml, raw_lambda_files: Dict[str, ByteString]
    ) -> IntegrationFamily:
    ...
    def _fetch_and_parse_serverless_yml_from_github(
        ...
        parsed_yml = parse_yml(raw_yml)

        self.parsed_yml = parsed_yml
        return parsed_yml
    ...
    def _fetch_raw_lambda_files_from_local(
        self, parsed_yml: SoclessParsedYml
    ) -> Dict[str, ByteString]:
        raw_lambda_files: Dict[str, ByteString] = {}
        raise NotImplementedError()
        return raw_lambda_files
